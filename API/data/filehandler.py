...
def load_json() -> Dict[str, Any]:
    try:
        ...
        raise ValueError("Nem található a fájl.")
...
def add_user(user: Dict[str, Any]) -> None:
    ...
    for u in data["Users"]:
        if u["id"] == user["id"]:
            raise ValueError("Létezik már ilyen ID-vel felhasználó")
...
def add_basket(basket: Dict[str, Any]) -> None:
    ...
    for letezo in data["Baskets"]:
        if letezo["id"] == basket["id"]:
            raise ValueError("Kosár már létezik ezzel az ID-val.")
